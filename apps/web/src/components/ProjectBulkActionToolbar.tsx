...
export type ProjectBulkAction = "FREEZE" | "UNFREEZE";

interface ProjectBulkActionToolbarProps {
  selectedCount: number;
  totalSelectable: number;
  selectionState: "none" | "some" | "all";
  onToggleSelectAll: () => void;
  onClearSelection: () => void;
  onAction: (action: ProjectBulkAction) => void;
  isExecuting: boolean;
}

export function ProjectBulkActionToolbar({
  selectedCount,
  totalSelectable,
  selectionState,
  onToggleSelectAll,
  ...
  onAction,
  isExecuting,
}: ProjectBulkActionToolbarProps) {
  const t = useTranslations("projects");

  const menuItems: DropdownMenuItem[] = (
    ["FREEZE", "UNFREEZE"] as const
  ).map((action) => ({
    label: action === "FREEZE" ? t("bulkFreeze") : t("bulkUnfreeze"),
    icon:
      action === "FREEZE" ? (
